...
verify preState maxIter rpcinfo maybepost = do
  ...
  smtState <- queryState
  tree <- Node (BranchInfo preState Nothing) . snd <$> interpret' (Fetch.oracle (Just smtState) rpcinfo model False) maxIter preState
  case maybepost of
    (Just post) -> do
...
equivalenceCheck bytecodeA bytecodeB maxiter signature' = do
  ...
  smtState <- queryState
  push 1
  aVMs <- Node (BranchInfo preStateA Nothing) . snd <$> interpret' (Fetch.oracle (Just smtState) Nothing SymbolicS False) maxiter preStateA
  pop 1
  push 1
  bVMs <- Node (BranchInfo preStateB Nothing) . snd <$> interpret' (Fetch.oracle (Just smtState) Nothing SymbolicS False) maxiter preStateB
  pop 1
